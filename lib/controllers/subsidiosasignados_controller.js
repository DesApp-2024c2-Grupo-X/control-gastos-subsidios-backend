...
export const getAllSubsidiosAsignados = async (req, res) => {
  ...
  res.json(subsidiosAsignadosEncontrados.map((subsidio) => subsidio.toJSON()));
};
...
export const getSubsidios = async (req, res) => {
  try {
    const subsidiosEncontrados = await SubsidiosAsignados.findAll({
      ...
      attributes: ['id', 'idProyecto', 'idRubro', 'montoAsignado'],
      include: [{ model: Rubros, as: 'Rubro' }],
    });

    // Mapea los resultados y agrega 'rubroNombre' como atributo
    const resultadosMapeados = subsidiosEncontrados.map((subsidio) => {
      return {
        id: subsidio.id,
        idProyecto: subsidio.idProyecto,
        idRubro: subsidio.idRubro,
        montoAsignado: subsidio.montoAsignado,
        rubroNombre: subsidio.Rubro.nombre,
      };
    });

    console.log('SUBENCONTRADOS2', resultadosMapeados);
